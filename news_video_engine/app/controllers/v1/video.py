...
@router.post(
    ...
    summary="Generate a short video from provided script",
)
def create_video_from_script(request: Request, body: ScriptVideoRequest):
    script = (body.video_script or "").strip()
    if not script:
        raise HttpException("", status_code=400, message="video_script is required")

    params = TaskVideoRequest(
        video_subject=(body.video_subject or "Daily News Update").strip() or "Daily News Update",
        video_script=script,
