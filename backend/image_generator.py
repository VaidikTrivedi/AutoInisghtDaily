...
def generate_background_image_prompt(agent:AIAgent, headline):
                        # ponytail: extract keywords only, avoid full headline (models render headlines as text)
                        words = [w for w in headline.lower().split() if len(w) > 4][:3]
                        theme = ' '.join(words) if words else 'news'
                        return f"abstract dark background gradient, {theme} mood, solid colors, atmospheric lighting"

def generate_background_image(agent:AIAgent, prompt, model):
                        open_router_agent = AIAgent(ai_provider=AIAgent.OPENROUTER, api_key= os.getenv("OPENROUTER_API_KEY"))
                        # ponytail: pass negative prompt to avoid text rendering
                        negative = "text, letters, words, typography, headlines, captions, subtitles, labels, watermarks"
                        image_url = open_router_agent.getAIImage(prompt=prompt, model=model, negative_prompt=negative)
                        if image_url:
                            try:
...
def generate_images(agent:AIAgent, news_summaries = None, run_locally=False, image_model=None):
    ...
    for news in news_summaries:
        # ponytail: retry up to 3 attempts per image, not whole batch
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    print(f"Failed post_{news['index']}.png after 3 attempts: {e}")
                    raise
